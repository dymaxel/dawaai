odoo.define('dxl_financial_dimensions.account_report_generic', function (require) {
'use strict';
...
accountReportsWidget.include({
    custom_events: _.extend({}, accountReportsWidget.prototype.custom_events, {
            ...
            'value_changed': function(ev) {
                ...
                self.report_options.bu = ev.data.bu;
                self.report_options.mob = ev.data.mob;
                self.report_options.toc = ev.data.toc;
                self.report_options.pc = ev.data.pc;
                self.report_options.loc = ev.data.loc;
                self.report_options.fun = ev.data.fun;
            ...
            if (!this.M2MFilters) {
                ...
                if (this.report_options.analytic_accounts) {
                    fields['bu'] = {
                        ...
                        value: this.report_options.bu.map(Number),
                        domain: [['level_id', 'in', this.report_options.bu_ids], ['id', 'in', this.report_options.user_bu]],
                    };
                    fields['mob'] = {
                        label: _t('Mode of Business'),
                        modelName: 'account.analytic.group',
                        value: this.report_options.mob.map(Number),
                        domain: [['parent_id', '=', this.report_options.bu[0] || []]],
                    };
                    fields['toc'] = {
                        label: _t('Type of Customers'),
                        modelName: 'account.analytic.group',
                        value: this.report_options.toc.map(Number),
                        domain: [['parent_id', '=', this.report_options.mob[0] || []]],
                    };
                    fields['pc'] = {
                        label: _t('Product Category'),
                        modelName: 'account.analytic.group',
                        value: this.report_options.pc.map(Number),
                        domain: [['parent_id', '=', this.report_options.toc[0] || []]],
                    };
                    fields['loc'] = {
                        label: _t('Location'),
                        modelName: 'account.analytic.group',
                        value: this.report_options.loc.map(Number),
                        domain: [['parent_id', '=', this.report_options.pc[0] || []]],
                    };
                    fields['fun'] = {
                        label: _t('Function'),
                        modelName: 'account.analytic.group',
                        value: this.report_options.fun.map(Number),
                        domain: [['parent_id', '=', this.report_options.loc[0] || []]],
                    };
                    fields['analytic_accounts'] = {
                        label: _t('Accounts'),
                        modelName: 'account.analytic.account',
                        value: this.report_options.analytic_accounts.map(Number),
                        /*domain: [['group_id', '=', this.report_options.fun[0] || []]],*/
                    };
                }
                ...
                var fun = this.report_options.fun[0];
                var loc = this.report_options.loc[0];
                var pc = this.report_options.pc[0];
                var toc = this.report_options.toc[0];
                var mob = this.report_options.mob[0];
                var bu = this.report_options.bu[0];
                var group_id = false;
                if (fun){
                    group_id = fun;
                } else if (loc){
                ...
                _.each(this.M2MFilters.widgets, function(field) {
                    /*if (field.name == 'analytic_accounts') {
                        field.record.fields.analytic_accounts.domain = ['|', ['group_id', '=', group_id], ['group_id', 'child_of', group_id]]
                    }*/
                    if (field.name == 'fun') {
                        field.record.fields.fun.domain = [['parent_id', '=', pc]]
                    }
                    if (field.name == 'loc') {
                        field.record.fields.loc.domain = [['parent_id', '=', pc]]
                    }
                    if (field.name == 'pc') {
                        field.record.fields.pc.domain = [['parent_id', '=', toc]]
                    }
                    if (field.name == 'toc') {
                        field.record.fields.toc.domain = [['parent_id', '=', mob]]
                    }
                    if (field.name == 'mob') {
                        field.record.fields.mob.domain = [['parent_id', '=', bu]]
                    }
                });
            }
        }
